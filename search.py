...
# to search for. For forensic purposes only.
...
def search(target, results_file):
    search_terms_input = input("Enter the search pattern (e.g., *.exe, *.py, enter for all files): ").strip()
    ...
    count = sum(1 for line in open(results_file))
